...
/* Configuration : MEM_METHOD
        ...
        MEM_STACK - to allocate the data block on tCORE_TICKS
...
#error "Please define a way to initialize a memory block."
        ...
        ee_printf("Total ticks (hex) : %08x%08x\n", (ee_u32)(total_time >> 32), (ee_u32)(total_time & 0xffffffffu));
        ee_printf("Total instrets (hex) : %08x%08x\n", (ee_u32)(total_instret >> 32), (ee_u32)(total_instret & 0xffffffffu));
        ee_printf("Total branchs (hex) : %08x%08x\n", (ee_u32)(total_branch >> 32), (ee_u32)(total_branch & 0xffffffffu));
        ee_printf("Total branch hits (hex) : %08x%08x\n", (ee_u32)(total_branch_hit >> 32), (ee_u32)(total_branch_hit & 0xffffffffu));
        ee_printf("IPC : %u.%06u\n", (ee_u32)(total_instret / total_time), (ee_u32)(((1000000 * total_instret) / total_time) % 1000000));
        ee_printf("Branch Hit Rate : %u.%06u%%\n", (ee_u32)((total_branch_hit * 100ull) / total_branch), (ee_u32)(((1000000000ull * total_branch_hit) / total_branch) % 1000000));
#if HAS_FLOAT
        ee_printf("Total time (secs): %f\n",time_in_secs(total_time));
        if (time_in_secs(total_time) > 0)
                ee_printf("Iterations/Sec   : %f\n",default_num_contexts*results[0].iterations/time_in_secs(total_time));
#else
        ee_printf("Total time (secs): %u.%06u\n",time_in_secs(total_time), time_in_secs(1000000*total_time) % 1000000);
        // if (time_in_secs(total_time) > 0)
        //      ee_printf("Iterations/Sec   : %d\n",default_num_contexts*results[0].iterations/time_in_secs(total_time));
                ee_printf("Iterat/Sec/MHz   : %u.%06u\n",(ee_u32)(default_num_contexts*((ee_u64)results[0].iterations)*USECS_PER_SEC/total_time),
                             (ee_u32)(1000000*default_num_contexts*((ee_u64)results[0].iterations)*USECS_PER_SEC/total_time % 1000000));
#endif
