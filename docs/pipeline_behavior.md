...
其中，输入级负责接收dispatch_integer_issue_port中的指令包，然后从Physical Register File中读取寄存器的有效状态，同时分析来自execute/wb流水级的反馈包，判断每个操作数的ready状态，对于仲裁出口不唯一的指令，还使用Round-Robin算法进行仲裁出口分配，并将结果分别送入发射队列及相关标志位向量
