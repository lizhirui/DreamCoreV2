...
namespace pipeline
{
    namespace execute
    {
        div::div(uint32_t id, component::handshake_dff<readreg_execute_pack_t> *readreg_div_hdff, component::port<execute_wb_pack_t> *div_wb_port) : tdb(TRACE_EXECUTE_DIV)
        {
            this->id = id;
            ...
            if(!commit_feedback_pack.flush)
            {
                if(!this->busy)
                {
                    if(!readreg_div_hdff->is_empty())
                    {
                        ...
                        assert(readreg_div_hdff->pop(&rev_pack));
                        ...
                        if(rev_pack.enable)
                        {
                            ...
                            assert(rev_pack.op_unit == op_unit_t::div);
